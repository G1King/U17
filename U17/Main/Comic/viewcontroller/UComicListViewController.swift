...
class UComicListViewController: UBaseViewController {
    ...
    private lazy var tableView: UITableView = {
        ...
        tw.uHead = URefreshHeader{ [weak self] in
            self?.loadData(more: false)
        }
        tw.uFoot = URefreshFooter {
            [weak self] in self?.loadData(more: true)
        }
        tw.uempty = UEmptyView{
         [weak self] in self?.loadData(more: false)
        }
        return tw
    }()
    ...
    private func loadData(more: Bool) {
        page = (more ? (page + 1) : 1)
        ApiLoadingProvider.request(API.comicList(argCon: argCon, argName: argName ?? "", argValue: argValue, page: page), model: ComicListModel.self) { [weak self] (returnData) in
